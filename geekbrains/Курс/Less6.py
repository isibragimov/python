...
def rotate_pistol(base_x, base_y, start):
    for i in range(start, random.randrange(7, 100)):
        phi_rad = PHI * i * math.pi / 180.0
        gotoxy(base_x + math.sin(phi_rad) * R, base_x + math.cos(phi_rad) * R + 60)
...
while answer != 'n':
    ...
    if answer == 'y':
        ...
        if start == 0:
            ...
            if z == 0:
                Less2.duble_files('test')
            elif z == 1:
                Less2.random_delete('test')
            else:
                gotoxy(-100, -50)
                turtle.write("Вам везет!", font=("Arial", 18, "normal"))
